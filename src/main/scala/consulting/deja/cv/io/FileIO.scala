...
object FileIO extends FileIO:
  private final case class WriterCharAppend(out: Writer, charset: Charset) extends CharAppend[WriterCharAppend]:
    override def append(ch: Char) =
      out.append(ch); this
    override def append(str: String) =
      out.append(str); this
    override def append(str: String, startIndex: Int, endIndex: Int) =
      out.append(str, startIndex, endIndex); this
    override def raw(str: String) =
      out.append(str); this
...
class FileIO private () extends IO[FileIO]:
  ...
  override def renderPDF(contents: CharAppendable, htmlFilesPrefix: String, pdfFile: String): FileIO =
    ...
    Seq(
      ...
      "--rm",
      "-v",
      s"${FileSystems.getDefault.getPath(".").toAbsolutePath}:/data",
      "pbaumgartner/weasyprint",
      mainHTMLFile,
      pdfFile).! match
      case 0 => this
