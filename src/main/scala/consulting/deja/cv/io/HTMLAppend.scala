...
object HTMLAppend:
  ...
  final case class BasedOnCharAppend[A <: CharAppend[A]](chars: A, language: Language)
  extends HTMLAppend[BasedOnCharAppend[A]]:
    ...
    override def tagOpen(label: String, attributes: (String, String)*): BasedOnCharAppend[A] =
      update(tagHead(label, attributes, chars) `append` ">")
    private def update(chars2: A): BasedOnCharAppend[A] = if chars eq chars2 then this else copy(chars = chars2)
  ...
  private def escapeHTMLAttributeValue[A <: CharAppend[A]](raw: String, out: A) =
    escape(raw, 0, htmlAttributeValueEscape, out)
  private def escapeHTMLLabel[A <: CharAppend[A]](raw: String, out: A) = escape(raw, 0, htmlLabelEscape, out)

  @tailrec private def escape[A <: CharAppend[A]](
    raw: String,
    startIndex: Int,
    fEscape: PartialFunction[Char, String],
    out: A): A =
    if startIndex == (raw length) then out
    else
      raw indexWhere (fEscape isDefinedAt, startIndex) match
