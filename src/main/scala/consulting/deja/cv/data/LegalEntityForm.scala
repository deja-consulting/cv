...
sealed trait LegalEntityForm:
  ...
  infix def addCommonAbbreviation(baseName: HTMLAppendable): HTMLAppendable
object LegalEntityForm:
  /** German "Aktiengesellschaft". */
  case object AG extends LegalEntityForm:
    override infix def addCommonAbbreviation(baseName: HTMLAppendable): HTMLAppendable =
      new HTMLAppendable:
        override def apply[A <: HTMLAppend[A]](append: A): A = append(baseName)(" ")(AGAcronym)

  /** Dutch "B.V." */
  case object DutchBV extends LegalEntityForm:
    override infix def addCommonAbbreviation(baseName: HTMLAppendable): HTMLAppendable =
      new HTMLAppendable:
        override def apply[A <: HTMLAppend[A]](append: A): A = append(baseName)(" ")(DutchBVAbbreviation)

  /** German GmbH */
  case object GmbH extends LegalEntityForm:
    override infix def addCommonAbbreviation(baseName: HTMLAppendable): HTMLAppendable =
      new HTMLAppendable:
        def apply[A <: HTMLAppend[A]](append: A): A = append(baseName)(" ")(GmbHAbbreviation)

  /** Company group, might consist out of several legal entities. */
  case object Group extends LegalEntityForm:
    override infix def addCommonAbbreviation(baseName: HTMLAppendable): HTMLAppendable =
      new HTMLAppendable:
        override def apply[A <: HTMLAppend[A]](append: A): A = append(baseName)(" ")(GroupEntityType)
