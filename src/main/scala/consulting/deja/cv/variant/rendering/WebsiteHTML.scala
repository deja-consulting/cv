...
import consulting.deja.cv.template.website.WebsiteTemplates.*
...
object WebsiteHTML:
  ...
  private lazy val skillsExpose: HTMLAppendable = SkillsExposeTable(
    OverviewSkillExpose.categories
      .flatMap {
        case (_, Left(_)) => Seq.empty
        case (_, Right(skillsByCategory)) => skillsByCategory
      }
      .map { case (category, skills) =>
        SkillsExposeTableRow(category, skills)
      }
  ...
  private def yearRange(fromYear: Int, untilYear: Int): HTMLAppendable = HTMLAppendable {
    ...
    if fromYear == untilYear then fromString
    else if fromYear + 1 == untilYear && sameCentury then s"$fromString/${untilString.takeRight(2)}"
    else if sameCentury then s"$fromString-'${untilString.takeRight(2)}"
  ...
  private sealed trait WrappedAppend[Base <: HTMLAppend[Base]] extends HTMLAppend[WrappedAppend[Base]]:
    ...
    def base: Base
    def withBase(newBase: Base): WrappedAppend[Base]
  ...
  private class FlatteningAppendRoot[Base <: HTMLAppend[Base]](val base: Base) extends WrappedAppend[Base]:
    override def withBase(newBase: Base): WrappedAppend[Base] = new FlatteningAppendRoot(newBase)
    override def tagAutoClose(label: String, attributes: (String, String)*): WrappedAppend[Base] =
      withBase(base.tagAutoClose(label, attributes*))
    override def tagClose(label: String): WrappedAppend[Base] =
      sys.error(s"Cannot close tag that was not opened: $label")
    override def tagOpen(label: String, attributes: (String, String)*): WrappedAppend[Base] = label match
      ...
      case _ => new FlatteningAppendChild(base.tagOpen(label, attributes*), this)
  ...
  extends FlatteningAppendRoot(base):
    override def withBase(newBase: Base): WrappedAppend[Base] =
