...
sealed trait Grammar:
  ...
  infix def enumeration[A <: HTMLAppend[A]](elements: Seq[HTMLAppendable], append: A): A = elements match
    case Seq() => append
    case Seq(onlyOne) => append(onlyOne)
    case Seq(first, second) => append(first)(" ")(And)(" ")(second)
    case Seq(first, rest*) => enumeration(rest, append(first)(", "))

  /** Combines a former name with its name today, after a name change. */
  def formerNameWithTodaysName[A <: HTMLAppend[A]](
    formerName: HTMLAppendable,
    todaysName: HTMLAppendable,
    append: A): A =
