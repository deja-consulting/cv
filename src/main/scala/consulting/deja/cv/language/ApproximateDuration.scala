...
final case class ApproximateDuration(duration: Duration) extends HTMLAppendable:
  import ApproximateDuration.*

  def apply[A <: HTMLAppend[A]](append: A): A =
    if duration.isNegative then ApproximateDuration(duration.negated)(append("-"))
    else
  ...
  private def appendUnit[A <: HTMLAppend[A]](
    days: Long,
    daysPerUnit: Int,
    singular: HTMLAppendable,
    plural: HTMLAppendable,
    append: A): A =
